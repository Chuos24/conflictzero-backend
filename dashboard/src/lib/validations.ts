...
export const registerSchema = z
  .object({
    ...
    contact_email: z.string().min(1, 'El email es requerido').email('Ingrese un email válido'),
    contact_phone: z
      .string()
      .optional()
...
export const addSupplierSchema = z.object({
  ...
  alert_threshold: z.number().min(1, 'El umbral mínimo es 1%').max(50, 'El umbral máximo es 50%'),
});
