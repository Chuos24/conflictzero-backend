...
export default function Privacy(): JSX.Element {
  ...
  const handleSubmitRequest = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      const res = await api.post(
        `/api/v2/audit/gdpr/requests?request_type=${selectedType}&description=${encodeURIComponent(description)}`
      );
  ...
  const handleRequestErasure = async (): Promise<void> => {
    if (
      !window.confirm(
        '¿Estás seguro de solicitar el borrado de tus datos? Esta acción iniciará un proceso de validación legal.'
      )
    ) {
      return;
    }
  ...
  if (loading) {
    return <div className="privacy-loading">Cargando...</div>;
  }
            ...
            {policies.map(policy => (
              <div key={policy.data_type} className="policy-card">
                ...
                <div className="policy-meta">
                  ...
                  <span className="policy-basis">📋 {policy.legal_basis}</span>
                </div>
                <div className="policy-tags">
                  {policy.allow_anonymization && <span className="tag tag-anon">Anonimizable</span>}
